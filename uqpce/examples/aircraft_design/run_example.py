...
class AeroStruct(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('aero', AeroDicipline(vec_size=vec_size), 
                           promotes_inputs= ['S', 'AR', 'V', 'rho', 'g', 'C_D0_base', 'ks_base', 'e_base', 'S_0', 'm_total'], 
                           promotes_outputs= ['LD', 'CL'])    
    
        self.add_subsystem('mass', Weights_Struct(vec_size=vec_size), 
                           promotes_inputs= ['S', 'AR', 'V', 'm_engine', 'm_total', 'm_fuse', 'kw_base', 'fsys_base', 'p_base', 'V_ref'], 
        ...
        bal.add_balance(name='m_fuel', eq_units='m', lhs_name='R_comp', rhs_name='R_target', units='kg')
...
if __name__ == '__main__':
    ...
    p.model.add_subsystem('engine_weight', EngineWeight(vec_size=resp_cnt), 
                        promotes_inputs= ['SFC_tech', 'm_eng_ref', 'alpha_base'], 
    ...
    p.model.add_subsystem('DOC', DOC(vec_size=resp_cnt),
                        promotes_inputs=['SFC_tech', ('V_cruise', 'V'), ('R', 'R_comp'), 'm_fuel', 'Cf_base', 'C_time', 'k_acq', 'C_eng_ref', 'beta_base', 'N_pax'],
    ...
    p.set_val('R_target', 5.765e6, units='m')
    p.set_val('N_pax', 162.)
    ...
    p.set_val('SFC_tech', 1.0)

    p.set_val('m_fuel', 18000.)
